...
void EUSCI_A2_UART_Init()
{
    ...
    //EUSCI_A2->BRW |= 1250; //9600 BAUD RATE
    EUSCI_A2->BRW |= 104; //115200 BAUD RATE
...
void EUSCI_A2_UART_InString(char *bufPt, uint16_t max)
{
    int length = 0;
    char character = EUSCI_A2_UART_InChar();
    ...
    while(character != CR)
    {
        ...
        else if (length < max)
        {
            *bufPt = character;
            bufPt++;
            length++;
            EUSCI_A2_UART_OutChar(character);
        }
        character = EUSCI_A2_UART_InChar();
    }
    *bufPt = 0;
}
